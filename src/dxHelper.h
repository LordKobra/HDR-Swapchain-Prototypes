...
inline HRESULT ReadDataFromFile(LPCWSTR filename, byte **data, UINT *size)
{
  ...
  {
    // Debug: Show which file is missing
    std::wstring errorMsg = L"Failed to open file: ";
    errorMsg += filename;

    // Show in message box so you definitely see it
    MessageBoxW(nullptr, errorMsg.c_str(), L"File Error", MB_OK | MB_ICONERROR);
    OutputDebugStringA("My output string.");
    std::string errorStr = "File not found: ";
    errorStr += LPCSTR(filename);
